...
task GetPotential{
  ...
  command<<<
    ...
    bedtools intersect -v -f 0.5 -wa -wb -a ~{name}.depth.ref.cluster.filt.bed -b ~{sd_blacklist} | bgzip -c > ~{name}.depth.ref.cluster.filt.rmSD.bed.gz
    bedtools intersect -v -f 0.5 -wa -wb -a ~{name}.depth.ref.cluster.filt.rmSD.bed.gz -b ~{igl_blacklist} | bgzip -c > ~{name}.depth.ref.cluster.filt.rmSD.rmIGL.bed.gz
