...
workflow CC_Crams{
    ...
    scatter (scatter_index in range(length(crams))){
        if (disable_dict_validation){
            call CollectCountsCramDisableValid as coverage_all_disable_valid{
                input:
    ...
    output {
        Array [String] sample_all = select_first([coverage_all_disable_valid.entity_id, coverage_all.entity_id])
        Array [File] counts_exons_all = select_first([coverage_all_disable_valid.counts_exons, coverage_all.counts_exons])
        Array [String] source_crams_all = select_first([coverage_all_disable_valid.source_cram, coverage_all.source_cram])
    }
}
...
task CollectCountsCramDisableValid {
    File intervals_exons
    File cram
    File crai
    File hg38_reference
