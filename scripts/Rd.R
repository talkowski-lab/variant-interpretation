...
plotJPG <- function(genotype_matrix,cnv_matrix,chr,start,end,cnvID,sampleIDs,allSamples,outputname,cnvtype,plotK,plotfamily,famfile,outFolder)
{
  ...
  if (plotfamily == TRUE ) {
    ...
    {
      family_id = family[i,1]
      ind_id = family[i,2]
      father_id = family[i,3]
      if(father_id == '0' | father_id == 0) {father_id = NA}
      mother_id = family[i,4]
      if(mother_id == '0' | mother_id == 0) {mother_id = NA}
      
      sibling_list <- family[which(family[, 4] == mother_id & family[, 3] == father_id & family[, 2] != ind_id), 2]
      children_list <- family[which(family[, 4] == ind_id | family[, 3] == ind_id), 2]
      #print(typeof(sibling_list))
      if(length(sibling_list) == 0){sibling_list = NA}
      if (length(sibling_list)>1){sibling_list = paste(sibling_list,collapse = ',')}
      if(length(children_list) == 0){children_list = NA}
      if (length(children_list)>1){children_list = paste(children_list,collapse = ',')}
      #strsplit(sibling_list, " ")
      #print(sibling_list)
      family[i,7] = sibling_list
    ...
    included <- samplesPrior
    #original_cnv_matrix<-cnv_matrix
    #original_genotype_matrix<-genotype_matrix
    ##Find samples to include##
    individual_list <-as.character(samplesPrior)
    sib_list <-as.character(family[which(family[,2] == samplesPrior), 7])
    child_list <-as.character(family[which(family[,2] == samplesPrior), 8])
    father_list <-as.character(family[which(family[,2] == samplesPrior), 3])
    mother_list <-as.character(family[which(family[,2] == samplesPrior), 4])
    ...
    for (i in included) {
      ...
      for (singlesample in na.omit(c(individual_list[count],strsplit(sib_list[count], ",")[[1]], strsplit(child_list[count], ",")[[1]], father_list[count],mother_list[count]))) {
        ...
        sampleID1s = unique(c(
          ...
          strsplit(child_list[count], ",")[[1]],
          individual_list[count]))
        #print(sampleID1s)
