...
final_output['START'] = final_output.apply(lambda x:  x.POS if x.Indel_type=='Insertion' else x.POS - x.LEN, axis=1).astype(int)
final_output['END'] = final_output.apply(lambda x:  x.POS + x.LEN if x.Indel_type=='Insertion' else x.POS, axis=1).astype(int)
