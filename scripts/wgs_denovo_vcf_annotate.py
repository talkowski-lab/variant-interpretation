...
df['TYPE'] = df.CSQ.apply(combine_csq, args=[csq_columns.index('VARIANT_CLASS')]).apply(lambda lst: pd.Series(lst).unique()[0]).map({'SNV': 'SNV', 'insertion': 'Indel', 'deletion': 'Indel'})
