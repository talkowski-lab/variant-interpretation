...
def load_variants(vcf_metrics_tsv, ultra_rare_inherited_tsv, var_type, use_random_gq=False, pl_filter=True): 
    ...
    ultra_rare[['PL_sample_0.0', 'PL_sample_0.1', 'PL_sample_1.1']] = ultra_rare.PL_sample.replace({np.nan: '[0, 0, 0]'}).str.strip('][').str.split(', ', expand=True).replace({'None': 0}).astype(int)
